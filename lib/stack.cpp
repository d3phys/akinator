...
$       (items  = (item_t *)realloc((void *)items, cap);)
        ...
        if (!items) {
                fprintf(logs, "Invalid stack reallocation: %s\n", strerror(errno));
                return nullptr;
        }
...
$       (err = verify_empty_stack(stk);)
        ...
        if (err) {
                fprintf(logs, "Can't construct (stack is not empty)\n");
                goto finally;
        }

        items = realloc_stack(stk, INIT_CAP);
        if (!items) {
                fprintf(logs, "Invalid stack memory allocation\n");
                err = STK_BAD_ALLOC;
                goto finally;
        }
...
$       (err = verify_stack(stk);)
        ...
        if (err) {
                fprintf(logs, "Can't push to invalid stack\n");
                goto finally;
        }
...
$               (void *items = realloc_stack(stk, capacity);)
                if (!items) {
                        fprintf(logs, "Invalid stack expanding: %s\n", strerror(errno));
                        err = STK_BAD_ALLOC;
                        goto finally;
                }
...
$       (err = verify_stack(stk);)
        ...
        if (stk->size == 0) {
                fprintf(logs, "Can't pop from an empty stack\n");
                err = STK_EMPTY_POP;
                goto finally;
        }
        if (shrinkable(stk)) {
...
$               (void *items = realloc_stack(stk, capacity);)
                if (!items) {
                        fprintf(logs, "Invalid stack shrinking: %s\n", strerror(errno));
                        err = STK_BAD_ALLOC;
                        goto finally;
                }
        ...
        if (stk->items == nullptr) {
...
#ifdef CANARY_PROTECT
                fprintf(logs, " Left  stack canary(hex) = %lx %s\n", stk->left_canary,
                                        indicate_err(vrf & INVALID_STK_LCNRY));

                fprintf(logs, " Right stack canary(hex) = %lx %s\n", stk->right_canary,
                ...
                for (size_t i = 0; i < stk->capacity; i++) {
                        if (stk->items[i] == POISON) {
                                fprintf(logs, "| 0x%.4lX stack[%7lu] = %p |\n", 
